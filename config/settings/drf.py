...
REST_FRAMEWORK = {
    ...
    # How many reverse proxies sit in front of Django. DRF needs it to find the
    # real client IP for the IP-keyed throttles (signup, login, MIE
    # registration): left unset, it keys on the whole X-Forwarded-For header,
    # which the client writes - so an IP limit can be escaped by varying it.
    # Set DRF_NUM_PROXIES per environment to the proxies actually in front of
    # the app (e.g. 1 behind a single load balancer). Unset keeps today's
    # behaviour, so nothing changes until an environment opts in.
    "NUM_PROXIES": config(
        "DRF_NUM_PROXIES",
        default=None,
        cast=lambda value: None if value in (None, "") else int(value),
    ),
...
SPECTACULAR_SETTINGS = {
    ...
    "ENUM_NAME_OVERRIDES": {
        ...
        # still invents a suffixed name without a pin.
        "DifficultyLevelEnum": "api.courses.enums.DifficultyLevel",
        # source_type — two models use this field name with different enums
