...
class MieSubmissionAdminViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    @extend_schema(
        ...
        description=(
            "Marks a submission as accepted. This works from any pipeline "
            "state — re-approving a previously rejected idea clears its "
            "rejection metadata and keeps any course already linked to it. "
            "Approval records the decision only: it does not create a "
            "course or move any money.\n\n"
            ...
            "**Auth:** The `mie.manage_console` and "
            "`mie.approve_topic_proposals` permissions together — Super Admin "
            "by default.\n\n"
            "**Prerequisites:** The submission must exist and be "
            "accessible.\n\n"
            "**Important:** Approval fires a SUBMISSION_APPROVED webhook "
            "to the developer immediately. Approval never pays the "
            "developer: payment is due only when a course produced from the "
            "idea is published. `payout_bypass` on the submission is "
            "recorded for that settlement but triggers nothing here. Re-approving a previously rejected "
            "submission clears all rejection metadata and keeps any "
            "linked course."
        ),
    ...
    @extend_schema(
        ...
        description=(
            ...
            "— a course already linked to the idea is left exactly as it "
            "is and stays linked, so a later re-approval finds it.\n\n"
            ...
            "developer immediately. A course linked from a previous "
            "approval is not changed: publication is one-way on the course "
            "side, so an admin handles a published course through the "
            "course tools. The decision's audit row names the linked "
            "course. This action works from any pipeline state including "
            "APPROVED."
        ),
        request=SubmissionDecisionSerializer,
        responses={
