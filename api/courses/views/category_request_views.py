...
ADMIN_ONLY_ACTIONS = {"approve", "reject"}


class CategoryRequestViewSet(ModelViewSet):
    """A creator's requests for a new Category, plus Admin approve/reject.

    Admins see every request (to review the queue); creators see only their
    own, mirroring CourseViewSet's creator-scoping pattern.
    """
    ...
    def reject(self, request, pk=None):
        category_request = category_request_service.reject_request(
            category_request=self.get_object(), admin=request.user
        )
        return Response(CategoryRequestSerializer(category_request).data)
