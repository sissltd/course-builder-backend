...
from api.wallets.services.bankaccount_services import AccountDetailsError
...
class BankAccountReadAccessTests(APITestCase):
	def setUp(self):
		...
		self.decrypt_patcher = patch(
			"api.wallets.serializers.bankaccount_serializers.decrypt_field",
	...
	def test_authenticated_user_lists_only_owned_accounts(self):
		mine = make_bank_account(user=self.creator)
		make_bank_account(user=make_user())
	...
	def test_admin_lists_all_accounts(self):
		make_bank_account(user=self.creator)
		make_bank_account(user=make_user())
	...
	def test_detail_returns_owned_account(self):
		account = make_bank_account(user=self.creator)
		self.client.force_authenticate(self.creator)
	...
	def test_detail_of_other_users_account_returns_404(self):
		account = make_bank_account(user=make_user())
		self.client.force_authenticate(self.creator)
...
class BankAccountCreateTests(APITestCase):
	...
	@patch("api.wallets.views.bankaccount_views.create_bank_account")
	def test_create_persists_and_returns_created_id(self, mock_create_bank_account):
		self.client.force_authenticate(self.creator)
		...
		self.assertIn("bank_account_id", response.data["data"])
		self.assertEqual(response.data["data"]["bank_account_id"], str(created.id))
	...
	@patch(
		"api.wallets.views.bankaccount_views.create_bank_account",
...
class BankAccountMutationTests(APITestCase):
	...
	def test_delete_soft_deletes_account(self):
		account = make_bank_account(user=self.creator, is_default=True)
	...
	def test_set_default_promotes_target_and_demotes_previous_default(self):
		first = make_bank_account(user=self.creator, is_default=True)
		second = make_bank_account(user=self.creator, account_number="1111111111")
...
class BankAccountSuspendAccessTests(APITestCase):
	"""Only IsAdminRole users can call suspend, and only for their own account."""

	def test_non_admin_role_cannot_suspend(self):
		creator = make_user(role=UserRole.COURSE_CREATOR)
		account = make_bank_account(user=creator)
	...
	def test_admin_can_suspend_any_account(self):
		...
		account = make_bank_account(user=acct_owner, is_default=True)
...
class VerifyBankAccountViewTests(APITestCase):
	def test_public_can_verify_bank_account(self):
		with patch(
			"api.wallets.views.bankaccount_views.PaystackService.resolve_bank",
...
class BankListViewTests(APITestCase):
	def test_public_can_fetch_bank_names_and_codes(self):
		with patch(
			"api.wallets.views.bankaccount_views.PaystackService.get_banks",
			return_value={
				"data": [
