...
class RedisNotificationPublishingTests(TestCase):
    def test_skips_publishing_when_cache_has_no_redis_url(self):
        with self.assertLogs("shared.redis.redis_service", level="DEBUG") as logs:
            asyncio.run(RedisService.publish_user_notification(uuid.uuid4(), "Test message"))

        self.assertIn("default cache has no Redis URL", logs.output[0])
