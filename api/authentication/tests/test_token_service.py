...
class IssueTokenTests(TestCase):
    def test_creates_hashed_token_and_returns_raw_token(self):
        user = make_user()
        record, raw_token = token_service.issue_token(
    ...
    def test_invalidates_prior_unused_token_for_same_user_and_purpose(self):
        user = make_user()
        first, _ = token_service.issue_token(
            user=user, purpose=TokenPurpose.SIGNUP_VERIFICATION
        )
        second, _ = token_service.issue_token(
            user=user, purpose=TokenPurpose.SIGNUP_VERIFICATION
        )
...
class CanResendTests(TestCase):
    ...
    def test_true_after_cooldown_elapses(self):
        user = make_user()
        record, _ = token_service.issue_token(
            user=user, purpose=TokenPurpose.SIGNUP_VERIFICATION
        )
